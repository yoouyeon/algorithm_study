...
```md
| 1234 | 최단경로 | [1234\_최단경로.js](Gold5/1234_최단경로.js) | [🔗](https://www.acmicpc.net/problem/1234) |
...
풀이 파일의 ANCHOR 주석에서 시작 시간을 읽어 현재 시간과 비교해 소요 시간을 계산한 뒤, 시작 시간을 제거하고 소요 시간을 추가한다.
...
- 백준 난이도: `bronze`, `silver`, `gold`, `platinum`, `diamond`, `ruby`
- 프로그래머스 난이도: `level0`, `level1`, `level2`, `level3`, `level4`, `unrated`
- 리트코드 난이도: `easy`, `medium`, `hard`
- 알고리즘 유형: 코드를 분석하여 자동으로 태그 추가 (예: `bfs`, `dfs`, `dp`, `greedy`, `binary-search` 등)

### 3. 코드 피드백

아래 두 가지 관점으로 피드백을 제공한다.
