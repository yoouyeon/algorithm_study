 ...
 * 재귀 호출은 양쪽 트리로 쪼개져서 병렬로 호출되기 때문에 (왼쪽 다 하고 오른쪽 검사) 콜 스택의 최대 깊이는 트리의 최대 깊이가 된다.
 * 균형트리의 경우에는 깊이가 log n이므로 O(log n)이겠지만
 * 최악의 경우 (왼쪽이나 오른쪽 노드만 있는 연결 리스트 형태의 트리) 콜 스택의 깊이가 n이 되어 O(n)
 */
